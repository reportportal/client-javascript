...
class RestClient {
    ...
    getRestConfig() {
        ...
        const config = Object.fromEntries(Object.entries(this.restClientConfig).filter(([key]) => key !== 'agent'));
    ...
    delete(path, data, options = { headers: this.headers }) {
        return RestClient.request('DELETE', this.buildPath(path), data, { ...options, ...this.getRestConfig() });
    }

    retrieveSyncAPI(path, options = { headers: this.headers }) {
        return RestClient.request('GET', this.buildPathToSyncAPI(path), {}, { ...options, ...this.getRestConfig() });
    }
}
