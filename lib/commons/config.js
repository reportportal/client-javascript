...
const getOAuthConfig = (options) => {
  // Support both nested oauth object and flat structure
  let oauthParams = options.oauth || {};

  // If oauth object doesn't exist, check for flat structure (backward compatibility)
  if (!options.oauth) {
    const { tokenEndpoint, username, password, clientId, clientSecret, scope } = options;
    oauthParams = { tokenEndpoint, username, password, clientId, clientSecret, scope };
  }

  const { tokenEndpoint, username, password, clientId, clientSecret, scope } = oauthParams;

  // Check if any OAuth params are provided
