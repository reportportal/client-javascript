...
class OAuthInterceptor {
  ...
  async requestToken(grantType) {
    ...
    // Determine if we're using any custom agent (from proxy or explicit config)
    const usingCustomAgent =
      Object.keys(proxyAgents).length > 0 ||
      this.restClientConfig.httpsAgent ||
      this.restClientConfig.httpAgent;
    ...
    const response = await axios.post(this.tokenEndpoint, params, {
      ...
      // This prevents axios from trying to apply its own proxy logic on top
      ...(usingCustomAgent && { proxy: false }),
    });
