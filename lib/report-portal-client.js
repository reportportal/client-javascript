...
class RPClient {
  ...
  constructor(options, agentParams) {
    ...
    if (this.config.batchLogs) {
      this.logBatcher = new LogBatcher(this.config.batchLogsSize, this.config.batchPayloadLimit);
    }
  }
  ...
  sendLogWithoutFile(itemTempId, saveLogRQ) {
    ...
    if (this.logBatcher) {
      return this.sendLogViaBatcher(itemObj, saveLogRQ, null);
    }
  ...
  sendLogWithFile(itemTempId, saveLogRQ, fileObj) {
    ...
    if (this.logBatcher) {
      return this.sendLogViaBatcher(itemObj, saveLogRQ, fileObj);
    }
  ...
  /**
   * Send log via batcher (batch logging mode).
   * @private
   * @param {Object} itemObj - item object from the map
   * @param {Object} saveLogRQ - log request data
   * @param {Object} fileObj - file object (optional)
   * @returns {Object} - an object which contains a tempId and a promise
   */
  sendLogViaBatcher(itemObj, saveLogRQ, fileObj) {
    const tempId = this.getUniqId();

    this.map[tempId] = this.getNewItemObj((resolve, reject) => {
      itemObj.promiseStart.then((itemResponse) => {
        const itemUuid = itemResponse.id;
        ...
        if (fileObj) {
          payload.file = { name: fileObj.name };
        }
        ...
        const batch = this.logBatcher.append(logRequest);

        if (batch) {
          this.sendLogBatch(batch).then(resolve, reject);
        } else {
          resolve({ queued: true });
        }
      }, reject);
  ...
  /**
   * Build multipart stream for batch logs.
   * @private
   * @param {Array} logRequests - array of log request objects
   * @param {string} boundary - multipart boundary
   * @returns {Buffer} - multipart stream buffer
   */
  buildBatchMultiPartStream(logRequests, boundary) {
    const eol = '\r\n';
    const bx = `--${boundary}`;
    const buffers = [];

    // Collect all payloads into a single JSON array
    const payloads = logRequests.map((logReq) => logReq.payload);

    // Add single JSON part containing array of all log payloads
    buffers.push(
      Buffer.from(
        `${bx}${eol}Content-Disposition: form-data; name="json_request_part"${eol}Content-Type: application/json${eol}${eol}${eol}${JSON.stringify(
          payloads,
        )}${eol}`,
      ),
    );

    // Add file parts for logs with attachments
    logRequests.forEach((logReq) => {
      const { file } = logReq;

      if (file && file.content) {
        buffers.push(
          Buffer.from(
            `${bx}${eol}Content-Disposition: form-data; name="file"; filename="${file.name}"${eol}Content-Type: ${file.type}${eol}${eol}`,
          ),
        );
        buffers.push(Buffer.from(file.content, 'base64'));
  ...
  /**
   * Send a batch of logs to ReportPortal.
   * @private
   * @param {Array} logBatch - array of log request objects
  ...
  sendLogBatch(logBatch) {
    ...
    this.logDebug(`Sending batch of ${logBatch.length} logs`);

    return this.restClient
      .create(url, this.buildBatchMultiPartStream(logBatch, boundary), {
        headers: {
          'Content-Type': `multipart/form-data; boundary=${boundary}`,
        },
      })
      .then((response) => {
        this.logDebug(`Successfully sent batch of ${logBatch.length} logs`, response);
        return response;
      })
      .catch((error) => {
        this.logDebug(`Error sending log batch`, error);
        console.dir(error);
        throw error;
      });
  }
  ...
  /**
   * Flush all pending logs from the batcher.
   * This is a public method that can be called to manually flush logs.
   *
   * @returns {Promise} - promise that resolves when all pending logs are sent
   */
  flushLogs() {
    if (!this.logBatcher) {
      return Promise.resolve();
    ...
    const batch = this.logBatcher.flush();
    return this.sendLogBatch(batch);
  }
