...
class RPClient {
  ...
  sanitizeData(data, { nameLimit, descriptionLimit } = {}) {
    const opts = {
      replaceBinaryChars: this.config.replaceBinaryChars,
      truncateFields: this.config.truncateFields,
      truncateAttributes: this.config.truncateAttributes,
    };
    if (nameLimit && data.name != null) {
      data.name = helpers.sanitizeField(data.name, nameLimit, opts);
    }
    if (descriptionLimit && data.description != null) {
      data.description = helpers.sanitizeField(data.description, descriptionLimit, opts);
    }
    if (data.attributes) {
      data.attributes = helpers.truncateAttributes(data.attributes, opts);
    }
    return data;
  }
  ...
  startLaunch(launchDataRQ) {
    ...
    if (launchDataRQ.id) {
      ...
      const rawAttributes = Array.isArray(launchDataRQ.attributes)
        ? launchDataRQ.attributes.concat(systemAttr)
        : systemAttr;
      const launchData = this.sanitizeData(
        {
        ...
        {
          nameLimit: this.config.launchNameLengthLimit,
          descriptionLimit: this.config.launchDescriptionLengthLimit,
        },
      );
  ...
  finishLaunch(launchTempId, finishExecutionRQ) {
    ...
    const finishExecutionData = this.sanitizeData(
      { endTime: this.helpers.now(), ...finishExecutionRQ },
      { descriptionLimit: this.config.launchDescriptionLengthLimit },
    );
  ...
  updateLaunch(launchTempId, launchData) {
    ...
    launchObj.promiseFinish.then(
      () => {
        ...
        this.logDebug(`Update launch with tempId ${launchTempId}`, sanitizedData);
        this.restClient.update(url, sanitizedData).then(
          (response) => {
  ...
  startTestItem(testItemDataRQ, launchTempId, parentTempId) {
    ...
    const testItemData = this.sanitizeData(
      {
        startTime: this.helpers.now(),
        ...testItemDataRQ,
      ...
      {
        nameLimit: this.config.itemNameLengthLimit,
        descriptionLimit: this.config.itemDescriptionLengthLimit,
      },
    );
  ...
  finishTestItem(itemTempId, finishTestItemRQ) {
    ...
    const finishTestItemData = this.sanitizeData(
      {
        endTime: this.helpers.now(),
