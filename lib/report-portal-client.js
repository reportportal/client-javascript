...
class RPClient {
  ...
  mergeLaunches() {
    ...
    this.logDebug('Option isLaunchMergeRequired is false, merge process cannot be done as no launch UUIDs where saved.');
  }
    ...
    launchObj.promiseFinish.then(
      () => {
        ...
        this.restClient.update(url, launchData, { headers: this.headers }).then(
          (response) => {
            this.logDebug(
              `Launch with tempId: ${launchTempId} were successfully updated`,
              response,
            );
