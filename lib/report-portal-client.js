...
const Analytics = require('./../analytics/anaytics');
...
class RPClient {
   ...
   */
    ...
    finishTestItemPromiseStart(itemObj, itemTempId, finishTestItemData) {
        const { launchUuid } = this;
        itemObj.promiseStart
            .then(() => {
                const url = ['item', itemObj.realId].join('/');
                this.logDebug(`Finish test item ${itemTempId}`);
                // eslint-disable-next-line max-len
                this.restClient.update(url, Object.assign(finishTestItemData, { launchUuid }), { headers: this.headers })
                    .then((response) => {
