   ...
   */
            ...
            .then(() => {
                launchObj.promiseStart
                    .then(() => {
                        ...
                        const url = [this.baseURL, 'launch', launchObj.realId, 'finish'].join('/');
    ...
    updateLaunch(launchTempId, launchData) {
        ...
        launchObj.promiseFinish.then(() => {
            const url = [this.baseURL, 'launch', launchObj.realId, 'update'].join('/');
    ...
    getRequestLogWithFile(saveLogRQ, fileObj) {
        ...
        return new Promise((resolve) => {
            rest.post(url, {
                data: this.buildMultiPartStream([saveLogRQ], fileObj, MULTIPART_BOUNDARY),
                headers: {
   ...
   */
    ...
    finishTestItemPromiseStart(itemObj, itemTempId, finishTestItemData) {
        itemObj.promiseStart
            .then(() => {
                const url = [this.baseURL, 'item', itemObj.realId].join('/');
                this.logDebug(`Finish test item ${itemTempId}`);
