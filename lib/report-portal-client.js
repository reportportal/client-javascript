...
class RPClient {
    ...
    Promise.all(launchObj.children.map((itemId) => this.map[itemId].promiseFinish)).then(
      () => {
        launchObj.promiseStart.then(
          async () => {
            if (this.config.batchLogs && this.logBuffer.length > 0) {
              this.logDebug(`Flushing ${this.logBuffer.length} buffered logs before finish launch`);
  ...
  async sendLogsBatch(logEntries) {
    ...
    try {
      ...
      this.logDebug(`Error sending batch of ${preparedEntries.length} logs`, error);
      console.dir(error);
