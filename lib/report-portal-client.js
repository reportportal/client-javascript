...
class RPClient {
  ...
  constructor(options, agentParams) {
    ...
    this.itemRetriesChainLatestTempIdMap = new Map();
    this.previousItemUuidMap = new Map();
  }
  ...
  cleanItemRetriesChain(tempIds) {
    tempIds.forEach((id) => {
      ...
      if (key) {
        ...
        this.itemRetriesChainLatestTempIdMap.delete(key);
      }

      this.itemRetriesChainKeyMapByTempId.delete(id);
      this.previousItemUuidMap.delete(id);
    });
  }
  ...
  startTestItem(testItemDataRQ, launchTempId, parentTempId) {
    ...
    // Track previous item for retry_of property
    if (testItemDataRQ.retry) {
      const previousItemTempId = this.itemRetriesChainLatestTempIdMap.get(itemKey);
      if (previousItemTempId) {
        this.previousItemUuidMap.set(tempId, previousItemTempId);
      }
    }
    ...
    this.itemRetriesChainLatestTempIdMap.set(itemKey, tempId);
  ...
  /**
     ...
     * Note: if the item is a retry, the "retry_of" property with the UUID of the previous retry is automatically included.
     * @Returns {Object} - an object which contains a tempId and a promise
     */
