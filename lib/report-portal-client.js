...
class RPClient {
    ...
    constructor(params) {
        ...
        this.isLaunchMergeRequired = params.isLaunchMergeRequired | false;
        this.map = {};
   ...
   */
                ...
                this.restClient.create(url, launchData, { headers: this.headers })
                    .then((response) => {
                        ...
                        if(this.isLaunchMergeRequired){
                            helpers.saveLaunchIdToFile(this.config.launch, responce.number, responce.id);
                        }
    ...
    mergeLaunches() {
        if (this.isLaunchMergeRequired) {
            let url = [this.baseURL, "launch", "merge"].join("/");
            let launchIds = helpers.readLaunchesFromFile();
            let data = this.getMergeLaunchesData(launchIds);
