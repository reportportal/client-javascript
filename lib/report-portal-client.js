...
class RPClient {
  ...
  constructor(options, agentParams) {
    ...
    this.logBuffer = [];
    this.logBufferPayloadSize = 0;
    this.pendingLogResolvers = new Map();
  ...
  finishLaunch(launchTempId, finishExecutionRQ) {
    ...
    launchObj.promiseStart
      .then(async () => {
        // First flush: prevent deadlock where child log promises wait for flush,
        // but flush waits for children to complete
      ...
      .then(async () => {
        // Second flush: ensure any remaining logs produced after the first flush are sent
        if (this.config.batchLogs) {
          await this.flushLogs();
        }
      })
  ...
  sendLog(itemTempId, saveLogRQ, fileObj) {
    ...
    if (this.config.batchLogs) {
      return this.addLogToBuffer(saveLogData, fileObj, itemTempId);
    }
  ...
  buildBatchMultiPartStream(logEntries, boundary) {
    const eol = '\r\n';
    const bx = `--${boundary}`;
    const buffers = [];

    const jsonLogs = logEntries.map((entry) => entry.logData);
    buffers.push(
      Buffer.from(
        `${bx}${eol}Content-Disposition: form-data; name="json_request_part"${eol}Content-Type: application/json${eol}${eol}${JSON.stringify(jsonLogs)}${eol}`,
      ),
    );
    ...
    logEntries.forEach((entry) => {
      if (entry.fileObj) {
        buffers.push(
          Buffer.from(
            `${bx}${eol}Content-Disposition: form-data; name="file"; filename="${entry.fileObj.name}"${eol}Content-Type: ${entry.fileObj.type}${eol}${eol}`,
          ),
        );
        buffers.push(Buffer.from(entry.fileObj.content, 'base64'));
        buffers.push(Buffer.from(eol));
      }
    });

    buffers.push(Buffer.from(`${bx}--${eol}`));

    return Buffer.concat(buffers);
  }

  calculateLogSize(logData, fileObj) {
    let size = Buffer.byteLength(JSON.stringify(logData), 'utf8');
    if (fileObj && fileObj.content) {
      size += Math.ceil((fileObj.content.length * 3) / 4);
    }
    return size;
  }

  addLogToBuffer(logData, fileObj, itemTempId) {
    const itemObj = this.map[itemTempId];
    if (!itemObj) {
      return this.getRejectAnswer(
        itemTempId,
    ...
    const tempId = this.getUniqId();
    const entrySize = this.calculateLogSize(logData, fileObj);

    if (entrySize > this.config.batchPayloadLimit) {
      console.warn(
        `Log entry size (${entrySize} bytes) exceeds batchPayloadLimit (${this.config.batchPayloadLimit} bytes). This may cause request rejection by the server.`,
      );
    }

    if (
      this.logBuffer.length > 0 &&
      this.logBufferPayloadSize + entrySize > this.config.batchPayloadLimit
    ) {
      this.enqueueBatch();
    ...
    this.pendingLogResolvers.set(tempId, { resolve: resolveLog, reject: rejectLog });
    ...
    this.logBufferPayloadSize += entrySize;
    itemObj.children.push(tempId);

    this.map[tempId] = this.getNewItemObj((resolve, reject) => {
      this.pendingLogResolvers.get(tempId).mapResolve = resolve;
      this.pendingLogResolvers.get(tempId).mapReject = reject;
    });
    const logObj = this.map[tempId];
    logObj.finishSend = true;
    logObj.promiseStart.then(
    ...
    if (this.logBuffer.length >= this.config.batchLogsSize) {
      this.enqueueBatch();
      this.flushLogs().catch(() => {});
    }
  ...
  enqueueBatch() {
    ...
    this.batchQueue.push([...this.logBuffer]);
    this.logBuffer = [];
    this.logBufferPayloadSize = 0;
  }
   ...
   * This method uses best-effort semantics: it attempts to send all buffered logs
   * but will not reject if some batches fail. Individual log promises will be
   * rejected on failure, but flushLogs() itself always resolves.
   * @returns {Promise<void>} Resolves when all buffered logs have been processed
   *                          (successfully sent or failed after retries).
   */
  async flushLogs() {
    while (true) {
      this.enqueueBatch();
      ...
      if (this.flushingLogs && this.flushPromise) {
      ...
      const logsToSend = this.batchQueue.shift();

      this.flushPromise = this.sendLogsBatch(logsToSend);

      try {
        await this.flushPromise;
      } catch {
        // Error already handled in sendLogsBatch (pendingLogResolvers rejected).
  ...
  async sendLogsBatch(logEntries) {
    ...
    for (const entry of logEntries) {
      try {
        await entry.itemObj.promiseStart;
        const itemUuid = entry.itemObj.realId;
        const isItemUuid = itemUuid !== this.launchUuid;
        ...
        if (entry.fileObj) {
          logData.file = { name: entry.fileObj.name };
        }
        ...
        const resolver = this.pendingLogResolvers.get(entry.tempId);
        if (resolver) {
          resolver.reject(error);
          if (resolver.mapReject) {
            resolver.mapReject(error);
          }
          this.pendingLogResolvers.delete(entry.tempId);
        }
      }
    }
    ...
    const hasFiles = preparedEntries.some((entry) => entry.fileObj);
    const url = 'log';
    const { batchRetryCount, batchRetryDelay } = this.config;
    let lastError;

    for (let attempt = 0; attempt <= batchRetryCount; attempt++) {
      try {
        ...
        if (attempt > 0) {
          this.logDebug(
            `Retry attempt ${attempt}/${batchRetryCount} for batch of ${preparedEntries.length} logs`,
          );
        } else {
          this.logDebug(
            `Sending batch of ${preparedEntries.length} logs${hasFiles ? ' with files' : ''}`,
          );
        }
        ...
        this.logDebug(`Successfully sent batch of ${preparedEntries.length} logs`, response);

        preparedEntries.forEach((entry) => {
          const resolver = this.pendingLogResolvers.get(entry.tempId);
          if (resolver) {
            resolver.resolve(response);
            if (resolver.mapResolve) {
              resolver.mapResolve(response);
            }
            this.pendingLogResolvers.delete(entry.tempId);
          }
        });
        ...
        if (attempt < batchRetryCount) {
          this.logDebug(
            `Batch send failed, will retry in ${batchRetryDelay}ms (attempt ${attempt + 1}/${batchRetryCount})`,
            error,
          );
    ...
    // All retries failed
    console.error(
      `Failed to send batch of ${preparedEntries.length} logs after ${batchRetryCount} retries:`,
      lastError,
    );
    ...
    preparedEntries.forEach((entry) => {
      const resolver = this.pendingLogResolvers.get(entry.tempId);
      if (resolver) {
        resolver.reject(lastError);
        if (resolver.mapReject) {
          resolver.mapReject(lastError);
        }
        this.pendingLogResolvers.delete(entry.tempId);
      }
    });
